...
fn count_combinations(file_path: &str, n: usize, extra_chars: &HashSet<char>) -> io::Result<DashMap<String, usize>> {
...
#[inline(always)]
fn filter(n: usize, threshold: usize, combinations: &DashMap<String, usize>, words: &DashMap<String, usize>, window: &mut Vec<char>) {
    let mut max_freq: usize = 0;
    let mut max_word = String::with_capacity(n);

    for i in 0..n {
        let word: String = (&window[i..i + n]).iter().collect();
        if let Some(freq) = combinations.get(&word) {
    ...
    let _ = window.split_off(n);
    if max_freq > 0 {
        *words.entry(max_word).or_insert(0) += 1;
    }
}

fn count_words(file_path: &str, n: usize, extra_chars: &HashSet<char>, threshold: usize, combinations: DashMap<String, usize>) -> io::Result<DashMap<String, usize>> {
    let file = File::open(file_path)?;
    let reader = BufReader::new(file);
        ...
        lines.into_par_iter().for_each(|line| {
            ...
            for c in line.chars() {
                if c >= '一' && c <= '鿿' {
                    window.push(c);
                    if window.len() == range {
                        filter(n, threshold, &combinations, &words, &mut window);
                    }
                } else {
                    window.clear();
                }
